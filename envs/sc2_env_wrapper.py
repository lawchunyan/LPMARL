...
class StarCraft2Env(MultiAgentEnv):
    ...
    def get_obs_enemy(self, enemy_id):
        ...
        if unit.health > 0:  # otherwise dead, return all zeros
            x = unit.pos.x
            y = unit.pos.y
            sight_range = self.unit_sight_range(0)

            # Movement features
            avail_actions = self.get_avail_agent_actions(0)
            for m in range(self.n_actions_move):
                move_feats[m] = avail_actions[m + 2]
    ...
    def get_obs(self):
        ...
        """
        if self.enemy_obs:
            agents_obs = [self.get_obs_agent(i) for i in range(self.n_agents)] + [self.get_obs_enemy(i) for i in
