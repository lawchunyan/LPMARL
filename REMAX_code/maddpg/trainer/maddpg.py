...
def p_train(make_obs_ph_n, act_space_n, p_index, p_func, q_func, optimizer, adversarial, adv_eps, adv_eps_s,
            num_adversaries, grad_norm_clipping=None, local_q_func=False, num_units=64, scope="trainer", reuse=None):
    with tf.variable_scope(scope, reuse=reuse):
        ...
        if adversarial:
            ...
            if p_index < num_adversaries:
                adv_rate = [adv_eps_s * (i < num_adversaries) + adv_eps * (i >= num_adversaries) for i in
                            range(num_agents)]
            else:
                adv_rate = [adv_eps_s * (i >= num_adversaries) + adv_eps * (i < num_adversaries) for i in
                            range(num_agents)]
...
def q_train(make_obs_ph_n, act_space_n, q_index, q_func, optimizer, adversarial, adv_eps, adv_eps_s, num_adversaries,
...
def vae_train(make_obs_ph_n, act_space_n, vae_index, vae_func_en, vae_func_de, vae_surrogate_func, optimizer,
              adversarial, adv_eps, \
    ...
    with tf.variable_scope(scope, reuse=reuse):
        ...
        encoder_loc = \
        vae_func_en(encoder_input, VAE_latent_dim, scope="vae_encoder_func", reuse=tf.AUTO_REUSE, num_units=num_units)[
            0]
        encoder_scale = \
        vae_func_en(encoder_input, VAE_latent_dim, scope="vae_encoder_func", reuse=tf.AUTO_REUSE, num_units=num_units)[
            1]
        ...
        surrogate_zt = vae_surrogate_func(surr_zt, 1, scope="surrogate_func", reuse=tf.AUTO_REUSE, num_units=num_units)[
                       :, 0]
        surrogate_func_vars = U.scope_vars(U.absolute_scope_name("surrogate_func"))

        surrogate_gradient = tf.gradients(surrogate_zt - gradient_reg * tf.norm(surr_zt),
                                          surr_zt)  # 1e-2 converge same points
        ...
        return train, encode, decode, prior, z_gradient, {'elbo_loss': elbo_print,
                                                          'surrogate_loss': surrogate_loss_print}


class MADDPGAgentTrainer(AgentTrainer):
    def __init__(self, name, model, obs_shape_n, act_space_n, agent_index, args, local_q_func, policy_name,
                 adversarial):
        self.name = name
...
class VAETrainer(AgentTrainer):
    def __init__(self, name, model_en, model_de, mlp_model, obs_shape_n, act_space_n, agent_index, args, local_q_func,
    ...
    def update(self, agents, t, t_step):
        ...
        for i in range(self.args.VAE_epoch):
            for j in range(int(len(self.B0UB1) / self.args.vae_batch_size)):
                ...
                target_q_next = [agents[jj].q_debug['target_q_values'](*(obs_next_n + target_act_next_n)) for jj in
                                 range(self.n)]
        ...
        for i in range(400):
            ...
            zt_init = zt_init + 0.1 * (
                        z_gradient[0] + self.args.surrogate_noise * np.random.normal(loc=0.0, scale=1 / (i + 1),
                                                                                     size=z_gradient[0].shape))
